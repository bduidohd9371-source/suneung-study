...
import ExamClearBoard from './ExamClearBoard.jsx';
...
export default function SubjectHub({ subject, onExit, onStartPractice, onUploadExam, onOpenExam, onOpenConcept }) {
  ...
  return <main className="hub-shell"><header className="hub-header"><button className="icon-button" onClick={onExit} aria-label="과목 목록으로"><ArrowLeft size={18} /></button><div><span className="eyebrow">SUBJECT STUDY SPACE</span><h1>{subject.name} 공부방</h1></div><span className={`subject-icon ${subject.color}`}>{subject.icon}</span></header>
    ...
    <ExamList subjectId={subject.id} onUpload={() => onUploadExam(subject)} onOpen={onOpenExam} />
...
function ExamList({ subjectId, onOpen, onUpload }) {
  ...
  return <><section className="hub-resource-section"><div className="hub-section-head"><div><span className="eyebrow">CSAT CLEAR BOARD</span><h2>년도별 기출 현황</h2><p className="hub-section-note">연도와 시험 종류를 선택해 올리면, 채점 완료 후 클리어 도장이 찍혀요.</p></div><button className="button button-secondary" onClick={onUpload}><Plus size={14} /> 시험지 추가</button></div><ExamClearBoard subject={{ id: subjectId }} exams={exams} onOpen={onOpen} /></section><section className="hub-resource-section"><div className="hub-section-head"><div><span className="eyebrow">QUESTION LIBRARY</span><h2>문제 목록</h2><p className="hub-section-note">올린 기출·모의고사 PDF를 선택해 원본 그대로 풀어요.</p></div></div>{exams.length ? <div className="hub-resource-list">{exams.map((exam) => <article className="hub-resource-row" key={exam.id}><button onClick={() => onOpen(exam)}><FileText size={17} /><span><strong>{exam.name}</strong><small>{exam.draft ? `이전 풀이 이어하기 · ${Object.keys(exam.draft.answers || {}).length}/${exam.total}개 답 저장` : `${exam.total}문항 · ${new Date(exam.createdAt).toLocaleDateString('ko-KR')}`}</small></span></button><span className="hub-resource-arrow">{exam.draft ? '이어 풀기' : '열기'}</span></article>)}</div> : <div className="hub-empty"><FileText size={22} /><p>저장된 시험지가 없어요. PDF 시험지와 정답표를 추가해 보세요.</p></div>}</section></>;
}
