...
export default function DueReviews({ onOpenPdf }) {
  ...
  return <section className="due-review-card"><div className="due-review-header"><div><span className="eyebrow">SPACED REVIEW</span><h2>오늘 복습할 문제</h2></div><span className="due-review-count"><AlarmClock size={14} /> {items.length}개 대기</span></div>
    {items.length ? <><p className="due-review-intro">틀렸던 문제를 다시 떠올린 뒤 정답을 확인하세요. 기억한 정도에 따라 다음 복습 날짜를 정합니다.</p><div className="due-review-list">{items.slice(0, 3).map((item) => {
      ...
      return <article className="due-review-item" key={item.key}><div className="due-review-item-head"><span>{attempt.subjectName} · {review.questionNumber}번</span><small>{attempt.source || '실전 풀이'} · {new Date(attempt.createdAt).toLocaleDateString('ko-KR')}</small></div><div className="review-schedule-row"><span className="review-dday">복습 {item.dueLabel}</span><div className="review-schedule-chips" aria-label="첫 오답일 기준 복습 예정일">{item.schedule.map((point) => <span key={point.day} className={point.label === '오늘' ? 'is-due' : ''}>+{point.day}일 <b>{point.label}</b></span>)}</div></div><p className="due-review-prompt">{prompt}</p>{question?.options && <ol className="due-review-options">{question.options.map((option, index) => <li key={index}>{index + 1}. {option}</li>)}</ol>}{review.insight && <p className="due-review-insight"><b>내 풀이 발상</b> {review.insight}</p>}{review.reason && <span className="due-review-reason">틀린 이유: {review.reason}</span>}
