...
export default function PdfImport({ onExit, onStart, onSaved, initialSubject = SUBJECTS[0] }) {
  ...
  async function saveExam(event) {
    ...
    try { await savePdfExam(exam); onSaved?.(exam); }
  ...
  return <main className="import-shell pdf-library-shell">
    ...
    <form className="pdf-setup-form" onSubmit={saveExam}>
      ...
      <button className="button button-primary pdf-start-button" disabled={saving || extracting || !file}><FileUp size={15} /> {saving ? '저장 중…' : '시험지 저장 후 과목방으로'}</button>
    </form>
