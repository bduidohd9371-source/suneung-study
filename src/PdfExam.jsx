...
export default function PdfExam({ exam, onExit, onOpenBank, viewOnly = false }) {
  ...
  const handlePdfPointerDown = (event) => {
    ...
    if (gesture.pointers.size === 2) {
      ...
      const pageLeft = frame ? (frame.width - pageSize.width) / 2 : 0;
      const focalX = (midpointX - (frame?.left || 0) - pageLeft - pdfPan.x) / Math.max(0.001, pdfZoom);
      const focalY = (midpointY - (frame?.top || 0) - pdfPan.y) / Math.max(0.001, pdfZoom);
      gesture.pinch = {
  ...
  const handlePdfPointerMove = (event) => {
    ...
    if (gesture.pointers.size >= 2 && gesture.pinch) {
      ...
      const frame = frameRef.current?.getBoundingClientRect();
      const pageLeft = frame ? (frame.width - pageSize.width) / 2 : gesture.pinch.pageLeft;
      const relativeMidX = midpointX - (frame?.left || 0);
      const relativeMidY = midpointY - (frame?.top || 0);
      const nextPanX = relativeMidX - pageLeft - gesture.pinch.focalX * nextZoom;
      const nextPanY = relativeMidY - gesture.pinch.focalY * nextZoom;
      setPdfZoom(Number(nextZoom.toFixed(3)));
      setPdfPan({ x: nextPanX, y: nextPanY });
      return;
    }
  ...
  const handlePdfWheel = (event) => {
    ...
    const nextZoom = Math.min(3, Math.max(0.5, pdfZoom + direction * 0.1));
    setPdfZoom(Number(nextZoom.toFixed(2)));
  };
