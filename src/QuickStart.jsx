...
import { Award, BookOpen, Check, Clock3, Pause, Play, RotateCcw, Sparkles, Target, X, Zap } from 'lucide-react';
    ...
    <div className="daily-quest-list">{quests.map((quest) => {
      ...
      return <article key={quest.id} className={`daily-quest ${completed ? 'completed' : ''} ${isActive ? 'in-progress' : ''}`}>
        ...
        <div className="daily-quest-info"><div className="daily-quest-title-row"><strong>{quest.title}</strong><span className="daily-quest-xp">{completed ? '완료 +5 XP' : '+5 XP'}</span></div><div className="daily-quest-meta"><span>{quest.subjectName}</span><span>·</span><span>{quest.cue}</span><span>·</span><span>{quest.wordQuest ? '무작위 미암기 단어' : <><Clock3 size={11} /> {quest.minutes}분</>}</span></div>
          ...
          {isActive && quest.wordQuest && wordSession && wordCards[wordIndex] && <div className="daily-word-session" role="dialog" aria-modal="true" aria-label="영단어 학습"><header className="daily-word-header"><div><span>영어 · 오늘의 일일 퀘스트</span><strong>수능 단어 복습</strong></div><button onClick={cancelSession} aria-label="단어 학습 종료"><X size={20} /></button></header><div className="daily-word-progress"><span>{wordIndex + 1} / {wordCards.length} · 오늘 평가 {wordSession.count}개 · {formatTime(wordSession.seconds)}</span><div className="quick-session-progress"><span style={{ width: `${(wordIndex / wordCards.length) * 100}%` }} /></div></div><div className="daily-word-card"><span className="eyebrow">{wordCards[wordIndex].sourceSection || wordCards[wordIndex].sourceName || '내 영어 단어장'}</span><strong className="daily-word-front">{wordCards[wordIndex].word}</strong>{wordRevealed ? <><p className="daily-word-meaning">{wordCards[wordIndex].meaning}</p><div className="daily-word-actions"><button onClick={() => rateQuestWord('again')}>기억안남</button><button onClick={() => rateQuestWord('learned')}>외움</button></div></> : <button className="button button-primary" onClick={() => setWordRevealed(true)}>뜻 확인</button>}</div><div className="daily-word-controls"><button className="button button-secondary" onClick={() => setRunning((value) => !value)}>{running ? <><Pause size={13} /> 잠깐 멈춤</> : <><Play size={13} /> 계속하기</>}</button><button className="daily-word-stop" onClick={cancelSession}><RotateCcw size={12} /> 그만하기</button></div></div>}
        </div>
