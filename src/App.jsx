...
import { ArrowRight, BookOpenCheck, CalendarDays, Check, Clock3, Moon, Sun, Target, Zap } from 'lucide-react';
...
function App() {
  ...
  return (
    <main className="app-shell">
      <header className="topbar"><a className="brand" href="#home" aria-label="수능 루틴 홈"><span className="brand-mark"><BookOpenCheck size={19} /></span><span>수능<span className="brand-light">루틴</span></span></a><div className="topbar-right"><span className="today-label">{dateLabel}</span><button className="calendar-shortcut" onClick={() => setCalendarActive(true)}><CalendarDays size={15} /> <span>달력·계획</span></button><button className="theme-toggle" onClick={toggleTheme} aria-label={theme === 'dark' ? '라이트 모드로 전환' : '다크 모드로 전환'} title={theme === 'dark' ? '라이트 모드' : '다크 모드'}>{theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}<span>{theme === 'dark' ? '라이트 모드' : '다크 모드'}</span></button><div className="avatar" aria-label="학생 프로필">수</div></div></header>
      <div className="page-content" id="home">
        ...
        <section className="section-heading"><div><span className="eyebrow">YOUR STUDY, YOUR PACE</span><h2>오늘 연습할 과목</h2></div><div className="section-heading-actions"><span className="section-note"><span className="live-dot" /> 과목을 선택하면 시험 시간이 설정돼요</span><button className="button button-secondary pdf-import-button" onClick={() => setBankActive(true)}><BookOpenCheck size={14} /> 문제 목록</button><button className="button button-secondary pdf-import-button" onClick={() => setPdfImportActive(true)}><BookOpenCheck size={14} /> PDF 시험지·OMR</button></div></section>
        ...
        <div className="bottom-grid"><ExamTimer subject={selected} /><aside className="focus-card"><div className="focus-head"><span className="focus-icon"><Zap size={17} /></span><span className="eyebrow">TODAY'S FOCUS</span></div><h3>선택한 과목으로<br />실전 연습 시작하기</h3><p>{selected.name} · {selected.minutes}분 시간 제한<br />제출하면 바로 채점해요.</p><button className="button button-primary start-exam-button" onClick={() => setExamActive(true)}>실전 풀이 시작 <ArrowRight size={16} /></button><button className="button button-secondary subject-hub-button" onClick={() => setHubActive(true)}>과목 공부방 열기 <ArrowRight size={15} /></button></aside></div>
        ...
        <Suspense fallback={null}><DueReviews onOpenPdf={async (id) => { try { const exam = await getPdfExam(id); if (exam) setPdfExam(exam); } catch { /* the PDF library can be reopened from the home screen */ } }} /></Suspense>
